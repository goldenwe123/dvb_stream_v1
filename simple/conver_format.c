...
int put_pes_header(char *packet,Pes_header *pes){
	...
	*/
	...
	packet[2]=temp;
	/*
	*Stream id Audio streams (0xC0-0xDF), Video streams (0xE0-0xEF)
	*/
	temp=pes->stream_id;
	packet[3]=temp;
	/*
	*If the PES packet length is set to zero, the PES packet can be of any length. 
	*A value of zero for the PES packet length can be used only 
	*when the PES packet payload is a video elementary stream.
	*/
	temp=0x00;
	packet[4]=temp;
	temp=0x00;
	...
	/*
	*Optional PES header
	*Usually the value is 0x80, 
	*which means that the data is not encrypted, 
	*has no priority, 
	*and is backed up data
	*/
	temp=pes->flag_priority;
	packet[6]=temp;
	/*
	*The value 0x80 means only pts is included, 
	*and the value 0xc0 means pts and dts are included
	*/
	temp=0x80;
	packet[7]=temp;
	/*
...
void h264_to_ts(MppPacket* packet,MppPacket* header,void (*func_ptr)(char*)){	
	Ts_header ts_header={
		...
		.sync_byte =  0x47, //Sync byte
		.transport_error_indicator = 0,//If 1, it means at least one error
	...
	char * header_ptr   = mpp_packet_get_pos(header);//pos is address of data start
