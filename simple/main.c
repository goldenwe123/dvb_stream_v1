...
void ts(char *ts){
	...
	//ret = sendto(outfd, buf, bufbase + 188, 0, outaddrs->ai_addr, outaddrs->ai_addrlen) < 0; 
	// if (ret) {
		// if (errno != EINTR) {
...
void *thread_input(void *arg)
{
	...
	do {
		...
		pkt_eos = decoder_read_packet(decoder);
		if( pkt_eos== -1)
				continue;
...
void *thread_encoder(void *arg)
{
	...
	do{
		...
		mpp_log("\rFPS: %f AVG: %d TIME: %d %d",calculate_fps(&encoder->frame_count),avg_fps,minute,sec);
...
int main(){
	...
	/* Input file */
	file_size=decoder_read_file("demo.h264",&fp_input);
	printf("size: %lu\n",file_size);
	...
	decoder_set_input(decoder,fp_input);
